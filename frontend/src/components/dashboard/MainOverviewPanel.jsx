...
export default function MainOverviewPanel({ data }) {
  ...
  const impactScore = data.model_prediction ?? 0;
  const isHighImpact = data.prediction_label ? data.prediction_label === "High Impact" : impactScore >= 0.5;
  const displayPrediction = data.prediction_label || (isHighImpact ? "High Impact" : "Low Impact");
  const impactScorePercentage = impactScore * 100;
  ...
  const summaryLeadBase = `This case is currently predicted as ${displayPrediction} with an impact score of ${impactScorePercentage.toFixed(0)}%.`;
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="rounded-lg border border-border bg-background/80 p-5 shadow-sm transition-shadow hover:bg-background hover:shadow-md">
          <div className="mb-4 flex items-center gap-2">
            ...
            <DashboardHelp text="Model's classification of the case's impact level and its impact score." />
          </div>
          ...
          <div className="mt-4">
            <div className="mb-2 flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                <span className="text-xs font-semibold text-muted-foreground">Impact score</span>
                <DashboardHelp text="Higher scores indicate stronger evidence for high impact; lower scores indicate stronger evidence for low impact." />
              </div>
              <span className="text-sm font-semibold text-foreground">{impactScorePercentage.toFixed(2)}%</span>
            </div>
            <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
              <div
                style={{ width: `${impactScorePercentage}%` }}
      ...
      <div className="rounded-lg border border-border bg-background/80 p-5 shadow-sm transition-shadow hover:bg-background hover:shadow-md">
        ...
        <div className="rounded-md border border-border bg-muted/30 px-4 py-4 text-sm leading-6 text-muted-foreground">
          <p className="font-semibold text-foreground">{summaryLeadBase}</p>
          <p className="mt-1">{humanRatingText}</p>

          <div className="mt-4 border-t border-border pt-3">
            <p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Technical details (optional)</p>
            <ul className="mt-2 list-disc space-y-1.5 pl-5">
              <li>
                {heatmapHighlights
                  ? <>Highest-weighted sentence: &quot;{heatmapHighlights.topSentence.text}&quot; (attention weight: {heatmapHighlights.topSentence.score}). Lowest-weighted sentence: &quot;{heatmapHighlights.worstSentence.text}&quot; (attention weight: {heatmapHighlights.worstSentence.score}).</>
                  : 'Sentence-level attention highlights are not available for this case.'}
              </li>
              <li>
                {featureHighlights
                  ? <>Strongest feature: {featureHighlights.topFeature.name} (importance: {featureHighlights.topFeature.score}). Weakest feature: {featureHighlights.worstFeature.name} (importance: {featureHighlights.worstFeature.score}).</>
                  : 'Feature importance highlights are not available for this case.'}
              </li>
              <li>
                {entityHighlights
                  ? <>Most represented entity category: {entityHighlights.highest.label} ({entityHighlights.highest.count} mentions). Least represented: {entityHighlights.lowest.label} ({entityHighlights.lowest.count} mentions).</>
                  : 'Entity count highlights are not available for this case.'}
              </li>
            </ul>
            <p className="mt-3">Explore the other tabs for the complete analysis.</p>
          </div>
        </div>
      </div>
    </div>
